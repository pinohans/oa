...
class  GovModel extends CommonModel {
	...
	private function _save_scope($data, $id, $user_list) {
		...
		if (!empty($return_user_list)) {
			...
			$sql .= ' select a.name,b.id,\''. $data['advice_id'].'\',\''. $data['advice'] . '\',0,a.id,'. time() .'  from ' . $this -> tablePrefix . 'user a, ' . $this -> tablePrefix . 'gov b where ' . $where;
