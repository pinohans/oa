...
class StudyController extends HomeController {
	...
	public function folder() {
		...
		$no_finish_task_count = badge_count_no_finish_study();
		$dept_task_count = badge_count_dept_study();
		$no_assign_task_count = badge_count_no_assign_study();
	...
	public function read($id) {
		...
		$task_log = M("StudyLog") -> where($where_log) -> select();
		...
		$task_accept = M("StudyLog") -> where($where_accept) -> find();
		...
		if ($this -> config['auth']['admin']) {
			...
			$task_dept_accept = M("StudyLog") -> where($where_dept_accept) -> find();
			if ($task_dept_accept) {
				$this -> assign('is_accept', 1);
		...
		$task_working = M("StudyLog") -> where($where_working) -> find();
	...
	function let_me_do($task_id) {
		if (IS_POST) {
			M("Study") -> where("id=$task_id") -> setField('executor', get_user_name() . "|" . get_user_id());
			M("Study") -> where("id=$task_id") -> setField('status', 10);
	...
	public function save_log($id) {
		...
		$task_id = M("StudyLog") -> where("id=$task_log_id") -> getField('task_id');
		...
		if ($status >= 20) {
			...
			$total_count = M("StudyLog") -> where($where_total_count) -> count();

			$where_finish_count['task_id'] = array('eq', $task_id);
			$where_finish_count['status'] = array('egt', 20);
			$finish_count = M("StudyLog") -> where($where_finish_count) -> count();

			if ($total_count == $finish_count) {
				M("Study") -> where("id=$task_id") -> setField('status', 30);

				$user_id = M('Study') -> where("id=$task_id") -> getField('user_id');

				$task = M("Study") -> where("id=$task_id") -> find();
				...
				$push_data['url'] = U('Study/read',"id={$task['id']}&return_url&Study/index");
		...
		if ($status == 21) {
			...
			D('Study') -> forword($task_id, $forword_executor);
		}
		if ($status == 22) {
	...
	private function _add_to_schedule($task_id) {
		$info = M("Study") -> where("id=$task_id") -> find();
		$data['name'] = $info['name'];
	...
	function _send_mail_finish($task_id, $executor) {
		...
		$transactor_name = M("StudyLog") -> where("task_id=$task_id") -> getField('id,transactor_name');
		...
		$body .= "点击查看任务详情：http://" . $_SERVER['SERVER_NAME'] . U('Study/read', 'id=' . $info['id']) . "</br>";
		$body .= "霞湖世家，感谢有您！</br>";
